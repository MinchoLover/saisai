...
import '../../../data/mock_tour_repository.dart';
...
class CandidatePage extends StatelessWidget {
  CandidatePage({super.key, required this.condition});
  final SearchCondition condition;
  final _repository = const MockTourRepository();
  final _planner = CoursePlanner();

  @override
  Widget build(BuildContext context) {
    final candidates = _planner.feasibleCandidates(
        condition, _repository.candidatesFor(condition));
    return Scaffold(
      body: Stack(
        children: [
          Positioned.fill(
                    ...
                    child: SingleChildScrollView(
                      ...
                      child: Row(
                        children: [
                          ...
                          ...condition.categories.map(
                              (category) => _ConditionChip(label: category)),
                        ],
                      ),
                    ),
                  ),
                      ...
                      itemBuilder: (context, index) {
                                  ...
                                  children: [
                                    Expanded(
                                      child: Column(
                                        ...
                                        children: [
                                          Text('${candidates.length}개의 사이사이 발견',
                                              style: const TextStyle(
                                                  fontSize: 21,
                                                  fontWeight: FontWeight.w900)),
                                          const SizedBox(height: 4),
                                          const Text('시간 안에 들를 수 있는 순서예요',
                                              style: TextStyle(
                        ...
                        return _CandidateCard(
                          ...
                          onTap: () => context.push('/place',
                              extra: PlaceDetailArgs(condition, place)),
                        );
                      },
                    ),
            ),
